<template>
  <div id="category_left">
    <div class="left_tab_item" v-for="(item, index) in list" :key="index" @click="itemClick(index)"
  ...
  export default {
    ...
    methods: {
      async getCategory() {
        ...
        const TempList = res.data.data.category.list
        console.log(TempList);
        const newList = TempList.splice(TempList.length - 1, 1);
        if (res) {
          this.list = TempList
...
<style scoped>
  ...
  .left_tab_item p {
    ...
    line-height: 0.8rem;
    /* line-height是用来控制border的,其父亲的line-height用来控制整体的行高，自己的用来控制元素的行高 */
